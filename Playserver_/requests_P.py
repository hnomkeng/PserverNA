...
def GETIMAGE(proxy):
    try:
        e.wait(timeout=0.2)
        requestid = (requests.post(url_getpic,verify=False, timeout=7, headers=header, proxies=proxy)).json()
        IMAGE_ID = requestid['checksum']
        e.wait(timeout=0.2)
        IMAGECT = requests.post(url_image + IMAGE_ID,verify=False, timeout=7,  headers=header, proxies=proxy)
...
def POSTIMAGE(self,votedata, proxy):
    for timeout in range(60):
        try:
            e.wait(timeout=0.2)
            vote = (requests.post(url_submitpic,verify=False, timeout=7,  headers=header, data=votedata, proxies=proxy)).json()
